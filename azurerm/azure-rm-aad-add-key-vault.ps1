...
if (!$adApplicationOnly)
{
    ...
    while ($count -lt $retryCount)
    {
        ...
        $error.Clear()
        $azurecert = Import-AzureKeyVaultCertificate -vaultname $vaultName -name $certNameInVault -filepath $pfxpath -password $securePassword
...
{
    $app = New-AzureRmADApplication -DisplayName $adApplicationName -HomePage $uri -IdentifierUris $uri -password $securePassword
