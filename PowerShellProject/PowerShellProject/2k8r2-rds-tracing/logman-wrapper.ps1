...
# ----------------------------------------------------------------------------------------------------------------
function populate-configFiles([ActionType] $action, [string[]] $configFiles)
{
...
function generate-config()
{
    ...
    # get base traces before adding new ones to export
     #run-logman  "query $($fullLoggerName) -ets"
     $output = run-logman  "query -ets"
     log-info $output
     #$regexPattern = "\n(?<set>[a-zA-Z0-9-_]*)\s*(?<type>\w*)\s*(?<status>\w*)"
     ...
     if($newList.Count -gt 0)
     {
         ...
         {
            $output = run-logman  "export $($session.Key) -ets -xml $(get-location)\$($session.Key).xml"
         }
     }
     else
     {
