...
.NOTES  
   ...
   History    : added kernel logger sessions using .\kernel subfolder
                
                changed folder time stamp
                added machine connectivity check
                added machine split 
...
$kernelEtwSessionName = "NT Kernel Logger"
$kernelEtwSessionNameFile = "$($kernelEtwSessionName).xml"
$useKernelEtwSession = $false
...
function main()
{
    ...
    # delete previous kernel logger xml files
    if([IO.File]::Exists($kernelEtwSessionNameFile))
    {
        [IO.File]::Delete($kernelEtwSessionNameFile);
    }
 
 
    # enumerate config files
    $configurationFiles = [IO.Directory]::GetFiles($configurationFolder,"*.xml",[IO.SearchOption]::TopDirectoryOnly)

    # enumerate kernel config files
    if([IO.Directory]::Exists("$($configurationFolder)\kernel"))
    {
        $kernelConfigurationFiles = [IO.Directory]::GetFiles("$($configurationFolder)\kernel","*.xml",[IO.SearchOption]::TopDirectoryOnly)
    }
 
    # populate configurationFiles from configuration directory
    populate-configFiles -configFiles $configurationFiles
    
    # determine if using kernel logger session
    if($kernelConfigurationFiles.Count -gt 0)
    {
        $useKernelEtwSession = $true
        log-info "configuring for kernel etw session"
        populate-configFiles -configFiles $kernelConfigurationFiles
    }
    ...
    if($useKernelEtwSession)
    {
        run-commands -currentAction $currentAction -configFiles @($kernelEtwSessionNameFile)
    }
...
function run-commands([ActionType] $currentAction, [string[]] $configFiles)
{
    ...
    foreach($machine in $machines)
    {
        ...
        {
            log-info "$($machine) not available. skipping."
            continue
        }
        ...
        if($gatherFolderFlat)
        {
            $gatherFolder = "$($workingDir)\$($defaultGatherFolder)\$($dirName)"
        }
        else
        {
            $gatherFolder = "$($workingDir)\$($defaultGatherFolder)\$($dirName)\$($machine)"            
        }
        ...
        foreach($file in $configFiles)
        {
            ...
            # dont modify session name if it is kernel logger
            if($file -ieq $kernelEtwSessionNameFile)
            {
                $fullLoggerName = $loggerName = $kernelEtwSessionName
            }
            else
            {
...
function populate-configFiles([string[]] $configFiles)
{
    ...
    foreach($file in $configFiles)
    {
        ...
        $xmlDoc.DocumentElement.LatestOutputLocation =  [string]::Empty #$outputFolder.ToString()
        $xmlDoc.DocumentElement.OutputLocation =  [string]::Empty #$outputFolder.ToString()
        $xmlDoc.DocumentElement.TraceDataCollector.FileName = [string]::Empty #$file.ToString()    
        ...
        $xmlDoc.DocumentElement.TraceDataCollector.LatestOutputLocation =  [string]::Empty #$outputFolder.ToString()
        ...
        if($useKernelEtwSession)
        {
            ...
            if(!([IO.File]::Exists($kernelEtwSessionNameFile)))
            {
                $xmlDoc.DocumentElement.Name = $kernelEtwSessionName
                $xmlDoc.DocumentElement.TraceDataCollector.Name = $kernelEtwSessionName
                $xmlDoc.DocumentElement.TraceDataCollector.SessionName = $kernelEtwSessionName
 
                xml-writer -file $kernelEtwSessionNameFile -xdoc $xmlDoc
                    
            }  
            else
            {
                [xml.xmldocument] $xmlDockernel = xml-reader $kernelEtwSessionNameFile
                $dupe = $false
 
                foreach($node in $xmlDoc.DocumentElement.TraceDataCollector.GetElementsByTagName("TraceDataProvider"))
                {
                    # check for dupes
                    foreach($t in $xmlDockernel.DocumentElement.TraceDataCollector.GetElementsByTagName("Guid")) 
                    {
                        if((($t.'#text').ToString()) -imatch ($Node.Guid.ToString()))
                        {
                            #dupe
                    ...
                    if(!$dupe)
                    {
                        $newNode = $xmlDockernel.ImportNode($node, $true)
                        $ret = $xmlDockernel.DocumentElement.TraceDataCollector.AppendChild($newNode)
                    }
                }
 
                xml-writer -file $kernelEtwSessionNameFile -xdoc $xmlDockernel
            }
        }
    }
}
...
function verify-configFiles()
{
    ...
    {
        $configurationFolder = "$($workingDir)$($configurationFolder.Substring(1))"
...
function generate-config()
{
    ...
    # get base traces before adding new ones to export
     ...
     for($i = 0; $i -lt $result.Count;$i++)
     {
        ...
        if(![System.String]::IsNullOrEmpty($loggerName))
        {
