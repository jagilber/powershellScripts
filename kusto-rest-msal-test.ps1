...
class KustoObj {
    ...
    [bool] AddIdentityPackageType([string]$packageName, [string] $edition) {
        ...
        $this.identityPackageLocation = @(get-childitem -Path $packageDirectory -Recurse | where-object FullName -match "lib\\$edition\\$packageName\.dll" | select-object FullName)[-1].FullName
      
        if (!$this.identityPackageLocation) {
            ...
            [string]$localPackages = . $this.nuget list -Source $nugetPackageDirectory
      
            if ($this.force -or !($localPackages -imatch $packageName)) {
                write-host "$($this.nuget) install $packageName -Source $nugetSource -outputdirectory $nugetPackageDirectory -verbosity detailed"
                . $this.nuget install $packageName -Source $nugetSource -outputdirectory $nugetPackageDirectory -verbosity detailed
                $this.identityPackageLocation = @(get-childitem -Path $packageDirectory -Recurse | where-object FullName -match "$edition\\$packageName\.dll" | select-object FullName)[-1].FullName
            }
            else {
                write-host "$packageName already installed" -ForegroundColor green
            }
        ...
        add-type -literalPath $this.identityPackageLocation
        return $true
    }
