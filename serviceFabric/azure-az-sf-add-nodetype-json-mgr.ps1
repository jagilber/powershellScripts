...
function main()
{
    ...
    if($validate)
    {
        if(!($resourcegroups = get-azresourcegroup))
        {
        ...
        Test-azResourceGroupDeployment -Verbose -ResourceGroupName $rg -TemplateFile $jsonOutputTemplateFile -TemplateParameterFile $jsonTemplateParameterFile

        # write-host "deploying template with -whatif"
        # new-azResourceGroupDeployment -Verbose -ResourceGroupName $rg -TemplateFile $jsonOutputTemplateFile -WhatIf
