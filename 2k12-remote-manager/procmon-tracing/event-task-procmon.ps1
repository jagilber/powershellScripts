...
function main()
{
    ...
    finally
    {
        ...
        log-info "finally"
        run-processes $eventTasksUnDeploy -wait $true
    }
}
...
function monitor-files()
{
    ...
    # will exit when a process that this script has started but NOT waited for has exited
    while(check-processList)
    {
        check-files
...
function check-files()
{
    ...
    #{
        ...
        foreach($file in $files)
        {
            ...
            {
                log-info "current files length: $($currentFiles.Length)"
                return
            }
        }
    #}
...
# ----------------------------------------------------------------------------------------------------------------
function run-processes($processes, [bool] $wait = $false)
{
    foreach($process in $processes.GetEnumerator())
    {
        $processId = run-process -processName $process.Name -arguments $process.Value -wait $wait
        if(!$processIds.ContainsKey($processId) -and $processId -ne 0)
        {
            log-info "adding process id to list: $($processId)"
            $processIds.Add($processId, $process.Name)
        }
    }
}
